...
class Forklift:
    ...
    def get_task(self):
        ...
        if self.task:
            ...
            response = requests.post(server_url + '/api/forklift/started_task', json=json.dumps(payload))
            #exit()
            ...
            payload = {
                'forklift_id': self.id,
                'warehouse_id': self.warehouse_id,
                'check_point_name': self.current_point['check_point_name'],
                'datetime': datetime.now().isoformat()
            }
            response = requests.post(server_url + '/api/forklift/reach_the_point', json=json.dumps(payload))
                ...
                if datetime.now() > self.next_point_time:
                    ...
                    if self.next_point:
                        ...
                        payload = {
                            'forklift_id': self.id,
                            'warehouse_id': self.warehouse_id,
                            'check_point_name': self.next_point['check_point_name'],
                            'datetime': datetime.now().isoformat()
                        }
                        response = requests.post(server_url + '/api/forklift/reach_the_point', json=json.dumps(payload))
                        #print(response.text)
                        ...
                        try:
                            ...
                            if self.path_direction == "forward":
                                ...
                                payload = {
                                    'forklift_id': self.id,
                                    'warehouse_id': self.warehouse_id,
                                    'target': self.current_path.target_rack_id,
                                    'datetime': datetime.now().isoformat()
                                }
                                response = requests.post(server_url + '/api/forklift/reach_the_target',
                    ...
                    # if we reach this else condition then forklift finish the task
                        ...
                        response = requests.post(server_url + '/api/forklift/finished_task',
                                                 json=json.dumps(payload))
                        #print(response.text)
                        #print('-' * 100)
